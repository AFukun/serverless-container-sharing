...
flags.DEFINE_string("output_file", "generated_graphs.json", "Output file name.")
flags.DEFINE_integer(
    "max_vertices", 7, "Maximum number of vertices including input/output."
)
...
def main(_):
    ...
    logging.info(
        "Using %d vertices, %d op labels, max %d edges",
        FLAGS.max_vertices,
        FLAGS.num_ops,
        FLAGS.max_edges,
    )
    for vertices in range(2, FLAGS.max_vertices + 1):
        for bits in range(2 ** (vertices * (vertices - 1) // 2)):
            ...
            matrix = np.fromfunction(
                graph_util.gen_is_edge_fn(bits), (vertices, vertices), dtype=np.int8
            )

            # Discard any graphs which can be pruned or exceed constraints
            if (
                not graph_util.is_full_dag(matrix)
        ...
        logging.info(
            "Up to %d vertices: %d graphs (%d without hashing)",
            vertices,
            len(buckets),
            total_graphs,
        )
