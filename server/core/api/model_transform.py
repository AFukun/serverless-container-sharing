...
from tensorflow.keras.layers import Conv2D, MaxPool2D, Dropout, Activation, Flatten, Dense, InputLayer, Add, \
    ZeroPadding2D, GlobalAveragePooling2D, AveragePooling2D, DepthwiseConv2D, ReLU, Reshape, Dropout
...
def resize_by_layer_info(oldTensorShape, newTensorShape, oldTensor, use_bias):
    ...
    for _ in newTensorShape:
        newTensorLength *= _
    tempOldTensor = tf.reshape(weight,[-1,])
...
def resize_bn_by_layer_info(old_layer_shape, new_layer_shape, old_layer, scale):
    ...
    if old_layer_length > new_layer_length:
        if old_layer.scale and scale:
            ...
            old_layer._gamma_const = K.constant(
                1.0, shape=(new_layer_shape[-1],))
        ...
        old_layer.moving_variance = tf.Variable(old_layer.moving_variance[:new_layer_length])
    else:
        if old_layer.scale and scale:
            old_layer.gamma = tf.Variable(tf.pad(old_layer.gamma, [[new_layer_length - old_layer_length, 0]]))
        elif old_layer.scale and scale == False:
            old_layer.gamma = None
            old_layer._gamma_const = K.constant(
                1.0, shape=(new_layer_shape[-1],))
        ...
        old_layer.beta = tf.Variable(tf.pad(old_layer.beta, [[new_layer_length - old_layer_length, 0]]))
        old_layer.moving_mean = tf.Variable(tf.pad(old_layer.moving_mean, [[new_layer_length - old_layer_length, 0]]))
        old_layer.moving_variance = tf.Variable(tf.pad(old_layer.moving_variance, [[new_layer_length - old_layer_length, 0]]))
...
def generate_edge(model, layers, model_info):
    ...
    for _ in range(1, childmodel_layers_length):
        ...
        if _ == 1:
            ...
            layers[_ - 1].inbound_nodes[0].output_tensors = tf.keras.Input(shape=model_info[_ - 1]['node_output_tensors_shape'])
        ...
        if len(inbound_layers_name) == 1:
            ...
            temp_inbound_layers_output_tensors = temp_inbound_layers.inbound_nodes[0].output_tensors
        else:
            ...
            for layer in temp_inbound_layers:
                temp_inbound_layers_output_tensors.append(layer.inbound_nodes[0].output_tensors)
        ...
        if len(layers[_].inbound_nodes) == 0:
            ...
            layers[_].inbound_nodes[0].input_tensors = temp_inbound_layers_output_tensors
            layers[_].inbound_nodes[0].inbound_layers = temp_inbound_layers
            if len(inbound_layers_name) > 1:
                layers[_].inbound_nodes[0].node_indices = [0 for _ in range(len(inbound_layers_name))]
                layers[_].inbound_nodes[0].tensor_indices = [0 for _ in range(len(inbound_layers_name))]
        # add new dict item
        inbound_layers[model_info[_]['layer_name']] = layers[_]
    # model's output_tensors setting
    x = tf.keras.Input(shape=model_info[childmodel_layers_length - 1]['node_output_tensors_shape'])
    ...
    model._init_graph_network(inputs=model.input, outputs=x, _layer=layers[childmodel_layers_length - 1])
    return model
...
def transform_by_layer_info(layer, layer_info):
    ...
    if layer_type == tf.keras.layers.Conv2D:
        ...
        resize_by_layer_info(old_layer_shape, new_layer_shape, layer, layer_info['layer_use_bias'])
        ...
        resize_bn_by_layer_info(old_layer_shape, new_layer_shape, layer, layer_info['layer_scale'])
        layer.name = layer_info['layer_name']
...
def swap_node_location(parentmodel_layers, node_to_node_mapping, model_info, model):
    ...
    for left in range(len(node_to_node_mapping)):
        stack = []
        if left != node_to_node_mapping[left][1] and _map[node_to_node_mapping[left][1]]:
            templeft = left
            _map[templeft] = False
            ...
            while (len(stack) > 0):
                ...
                parentmodel_layers[node_to_node_mapping[pop_element][1]] = parentmodel_layers[pop_element]
            parentmodel_layers[node_to_node_mapping[left][1]] = tempmodule
    childmodel = generate_edge(model, parentmodel_layers[0:len(model_info)], model_info)
    return childmodel
...
def model_structure_transformation(parentModel, childmodel_info, node_to_node_mapping):
    ...
    # add_modules
    for _ in range(parentmodel_layers_length, parentmodel_layers_length + childmodel_layers_length):
        if node_to_node_mapping[_][1] >= childmodel_layers_length:
            parentmodel_layers.append([])
        else:
            layer_info = childmodel_info[node_to_node_mapping[_][1]]
            if layer_info['layer_type'] == 'InputLayer':
                layer = InputLayer(input_shape=(224, 224, 3,), name='input_1')
            elif layer_info['layer_type'] == 'Flatten':
                layer = Flatten()
            elif layer_info['layer_type'] == 'Conv2D':
                layer = Conv2D(name=layer_info['layer_name'], filters=layer_info['layer_filters'], kernel_size=layer_info['layer_kernel_size'], strides=layer_info['layer_strides'],
                                padding=layer_info['layer_padding'], activation=layer_info['layer_activation_name'], use_bias=layer_info['layer_use_bias'])
            elif layer_info['layer_type'] == 'Dense':
                layer = Dense(name=layer_info['layer_name'], units=layer_info['layer_units'], activation=layer_info['layer_activation_name'])
            elif layer_info['layer_type'] == 'MaxPool2D':
                layer = MaxPool2D(name=layer_info['layer_name'], pool_size=layer_info['layer_pool_size'], strides=layer_info['layer_strides'])
            elif layer_info['layer_type'] == 'AveragePooling2D':
                layer = AveragePooling2D(name=layer_info['layer_name'], pool_size=layer_info['layer_pool_size'], strides=layer_info['layer_strides'], padding=layer_info['layer_padding'])
            elif layer_info['layer_type'] == 'BatchNormalization':
                layer = tf.compat.v1.keras.layers.BatchNormalization(name=layer_info['layer_name'], axis=layer_info['layer_axis'], scale=layer_info['layer_scale'], epsilon=layer_info['layer_epsilon'])
            elif layer_info['layer_type'] == 'Add':
                layer = Add(name=layer_info['layer_name'])
            elif layer_info['layer_type'] == 'ZeroPadding2D':
                layer = ZeroPadding2D(name=layer_info['layer_name'], padding=layer_info['layer_padding'])
            elif layer_info['layer_type'] == 'Activation':
                layer = Activation(layer_info['layer_activation_name'], name=layer_info['layer_name'])
                ...
                layer = tf.compat.v1.keras.layers.Concatenate(name=layer_info['layer_name'], axis=layer_info['layer_axis'])
            elif layer_info['layer_type'] == 'DepthwiseConv2D':
                layer = DepthwiseConv2D(name=layer_info['layer_name'],
                ...
                layer = ReLU(name=layer_info['layer_name'], max_value=layer_info['layer_max_value'])
            elif layer_info['layer_type'] == 'Reshape':
                layer = Reshape(name=layer_info['layer_name'], target_shape=layer_info['layer_target_shape'])
            elif layer_info['layer_type'] == 'Dropout':
                layer = Dropout(name=layer_info['layer_name'], rate=layer_info['layer_rate'])
            else:
                raise Exception("This type has not been added")
            parentmodel_layers.append(layer)

    # step 2: transformation by edge
    childModel = swap_node_location(parentmodel_layers, node_to_node_mapping, childmodel_info, model)
