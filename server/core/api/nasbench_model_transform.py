...
def generate_edge(model, layers, model_info):
    ...
    for _ in range(1, childmodel_layers_length):
        ...
        temp_inbound_layers = None
        if len(inbound_layers_name) == 1:
            if inbound_layers_name[0] in inbound_layers.keys():
                ...
                temp_inbound_layers_output_tensors = tf.keras.Input(shape=model_info[_]['node_input_shapes'])
            # temp_inbound_layers = inbound_layers[inbound_layers_name[0]]
            ...
            if len(temp_inbound_layers) == 0:
                temp_input_tensors = []
                for i, input_shape in enumerate(model_info[_]['node_input_shapes']):
                    temp_input_tensors.append(tf.keras.Input(shape=input_shape))
                temp_inbound_layers_output_tensors = temp_input_tensors
