    ...
    try:
        ...
        for k, v in model_metadata.items():
            if isinstance(v, (dict, list, tuple)):
                f.attrs[k] = json.dumps(v, default=serialization.get_json_type).encode(
                    "utf8"
                )
            else:
                f.attrs[k] = v
...
def load_model_from_hdf5(
    ...
    try:
        ...
        read_json_time = end - start
        print("read json:{}".format(end - start))
        start = time.time()
        model = model_config_lib.model_from_config(
            model_config, custom_objects=custom_objects
        )
        end = time.time()
        rescontruct_time = end - start
        print("rescontruct model struct time:{}".format(end - start))
        # set weights
        start = time.time()
        load_weights_from_hdf5_group(f["model_weights"], model.layers)
        end = time.time()
        load_weights_time = end - start
        print("load_weights time:{}".format(end - start))
        with open("output.txt", "a") as f:
            print(
                read_json_time, "\t", rescontruct_time, "\t", load_weights_time, file=f
            )
        if compile:
...
def save_attributes_to_hdf5_group(group, name, data):
    ...
    if bad_attributes:
        raise RuntimeError(
            ...
            % (HDF5_OBJECT_HEADER_LIMIT, ", ".join([x for x in bad_attributes]))
        )
