...
def test_resnet50_add(model, model_info):
    ...
    for _, layer_info in enumerate(model_info):
        ...
        during = end - start
        record.append([during])
    return record


def test_resnet50_fail(model):
    layers = []
    record = []
...
def test_resnet50_connect(model):
    record = []
    for _, layer in enumerate(model.layers):
        if _ == 0 or _ == 1:
            continue
        ...
        if type(layer.inbound_nodes[0].inbound_layers) == list:
            for inbound_layer in layer.inbound_nodes[0].inbound_layers:
                input_tensors.append(inbound_layer.output)
            for inbound_layer in layer.inbound_nodes[0].inbound_layers:
                input_layers.append(inbound_layer)
        else:
            input_tensors = layer.inbound_nodes[0].inbound_layers.output
            input_layers = layer.inbound_nodes[0].inbound_layers

        model.layers[_].inbound_nodes[0].input_tensors = input_tensors
        model.layers[_].inbound_nodes[0].inbound_layers = input_layers
        end = time.time()
        during = end - start
        record.append([during])
    return record
...
def test_resnet50_reshape(model, model_info):
    conv2d_index = []
    bn_index = []
    record = []
    for _, layer in enumerate(model.layers):
        if type(layer) == Conv2D:
            conv2d_index.append(_)
        if type(layer) == tf.compat.v1.keras.layers.BatchNormalization:
            bn_index.append(_)
    for _, layer in enumerate(model.layers):
        if type(layer) == Conv2D:
            for index in conv2d_index:
                start = time.time()
                transform_by_layer_info(layer, model_info[index])
                end = time.time()
                during = end - start
                record.append([during])
                transform_by_layer_info(layer, model_info[_])
    for _, layer in enumerate(model.layers):
        if type(layer) == tf.compat.v1.keras.layers.BatchNormalization:
...
def test_resnet50_replace(model):
    record = []
    ...
    for _, layer in enumerate(model.layers):
        ...
        if type(layer) == Conv2D or type(layer) == Dense:
            if type(layer) == Conv2D:
                input_channel = layer.kernel.shape[2]
            else:
                input_channel = layer.kernel.shape[0]
            ...
            x = layer(Inputs)
            sub_model = Model(inputs=Inputs, outputs=x)
            temp_record.append(sub_model.layers[1].kernel.shape)
            sub_model.save_weights(r"save_weights.h5")
            start = time.time()
            sub_model.load_weights(r"save_weights.h5")
            end = time.time()
            temp_record.append(end - start)
...
# add
model = tf.keras.models.load_model("/data/resnet50.h5")
record = test_resnet50_add(model, model_info)
record_data("add.csv", "add", record)
...
# connect
model = tf.keras.models.load_model("/data/resnet50.h5")
record = test_resnet50_connect(model)
record_data("connect.csv", "connect", record)

# fail
model = tf.keras.models.load_model("/data/resnet50.h5")
record = test_resnet50_fail(model)
record_data("fail.csv", "fail", record)

# reshape
model = tf.keras.models.load_model("/data/resnet50.h5")
record = test_resnet50_reshape(model, model_info)
record_data("reshape.csv", "reshape", record)
